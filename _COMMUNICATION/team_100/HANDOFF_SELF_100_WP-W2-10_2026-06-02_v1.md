# Session Handoff — team_100 | Track-2 elevation intake complete; orchestrate WP-W2-10 A/B/E/F implementation through S3 build -> S4 token-compliance -> S5 QA + L-GATE_VALIDATE per AOS procedures, using a PHASED model (not all-at-once). A S2-signed (S3 unblocked); B/E/F READY_FOR_S2 (team_00 proxy). LOD400_IMPL specs authored. Phased model: Phase0 shared prep (assets+context-driven partials) serial; Phase1 A SOLO pattern-setter; Phase2 B/E/F parallel S3 in isolated worktrees; Phase3 serialize FTP deploy + roadmap.yaml(IR#4) + Cursor validates. QA: axe http, LH perf https, mobile median>=85, a11y100, axe 0 crit/serious; gate-order strict. D-14 composition-only (GCR if new token). ADR034 offline-fallback; main@c95b6d2; Track-1 WP-W2-11/12/13 DONE+archived.


## 1. SESSION ACCOMPLISHED
- Track-2 elevation package intake (A/B/E/F + real assets)
- Authored 4 implementation LOD400s (WP-W2-10-A/B/E/F)
- Recorded A S2 sign-off (team_00 proxy)
- Closed+archived WP-W2-11/12/13
...
# 8. MISSION ADDENDUM — Track-2 Full-Parallel Orchestration Runbook (team_100)

> Authored by team_100 to carry the specific session mission the canonical scaffold (Sections 1–7) does not.
> team_00 directive (2026-06-02): orchestrate the 4 Track-2 clusters through the complete gate chain incl. tests, per procedures — using the **PHASED model** (A pattern-setter first → B/E/F parallel → serialized integration), NOT a simplistic all-at-once 4-way build.

## Mission
Implement WP-W2-10 Track-2 high-visual clusters **A, B, E, F** end-to-end: **S3 build → S4 token-compliance → S5 (team_50 L-GATE_BUILD → team_190 L-GATE_VALIDATE) → close + archive**, run in parallel, per the implementation LOD400s:
`_aos/work_packages/S003/WP-W2-10-{A,B,E,F}/LOD400_IMPL_spec.md`. G (Hero video) BLOCKED (Eyal asset).

| Cluster | Template(s) | Routes | S2 state |
|---|---|---|---|
| A Service | tpl-service | /treatment /method /sound-healing /lessons | **S2 SIGNED** (SIGNOFF_S2_WP-W2-10-A_2026-06-02.md) → S3 unblocked |
| B Editorial | tpl-content | /about /press /about/moksha | READY_FOR_S2 (team_00 proxy ok) |
| E Commerce | tpl-books /-detail /-shop-* | /books +3 details /shop +5 | READY_FOR_S2 |
| F EN | tpl-en-landing | /en (LTR mirror) | READY_FOR_S2 |

Source-of-truth designs + real assets: `_COMMUNICATION/team_35/WP-W2-10-{A,B,E,F}/elevation/` + `handoff-WP-W2-10-Track2/shared-assets/` (portrait, 3 book covers, gallery, logo, hero-wide).

## Implementation model (architect-recommended — PHASED, not all-at-once)
> Correction (team_00, 2026-06-02): build in stages, NOT a cold 4-way parallel. A goes first as the
> pattern-setter so the render-fn pattern + parametrized partials are proven once; B/E/F then parallelize on top.

**Phase 0 (SERIAL, once):** (a) add shared real assets to theme media `site/wp-content/themes/ea-eyalamit/assets/images/`; (b) make reused block partials (hero, .ea-pillar/method-pillars, bio-block, testimonials, contact-cta, footer, topnav) **context-driven** so each cluster only *calls* them. Removes the #1 collision risk.
**Phase 1 (A SOLO — pattern-setter):** build cluster **A** first (S2-signed). A establishes the route-aware
`ea_wave2_render_service_blocks()` pattern and proves the parametrized shared partials. Take A through
**S3 → S4 → deploy → S5** (or at minimum S3+S4+deploy) and capture the reusable pattern. Fix-once, not four-times.
**Phase 2 (B/E/F PARALLEL — only after A's pattern is proven):** confirm B/E/F S2 sign-off (team_00 proxy), then
spawn **3** team_10 build sub-agents with **`isolation: worktree`** (separate git worktrees → no collision), each
touching ONLY its own template + provider + cluster CSS sheet (B=ea-blog, E=w2-05-shop, F=w2-08-en-landing).
**Phase 3 (SERIALIZE integration — 3 hard conflict points, applies throughout):**
  1. **FTP staging deploy** — single target; deploy clusters one at a time (never concurrent `ftp_deploy_site_wp_content.py`).
  2. **roadmap.yaml** — Iron Rule #4 single writer = team_100; serialize all status/gate writes.
  3. **Cross-engine validates** — team_190 in **Cursor** (no Codex; team_00-approved); sequence them; gate-order team_50 PASS before team_190.

## Gate chain per cluster (strict order)
S3 (team_10, Claude) → S4 (team_80 token-compliance: zero D-14 drift; GCR to team_100 if a new token/atom is truly needed) → **deploy (serialized)** → S5 team_50 L-GATE_BUILD → **only on PASS** → team_190 L-GATE_VALIDATE (Cursor, cross-engine). team_100 pre-flight (puppeteer/QA) before routing externally is recommended (catches issues early, as in Track-1).

## QA / acceptance (every cluster)
- **axe** over http: 0 critical / 0 serious on all cluster routes.
- **Lighthouse** perf over **https** (production-representative — WP-W2-11 disposition; http→https redirect artifact), **mobile triple-run median ≥85**, **a11y 100**.
- Composition matches the elevated mockup; **zero new D-14 tokens/atoms**; single H1/route; RTL (F = LTR mirror via logical props); graceful Eyal-gap placeholders (sand-circle avatars, gradient hero); real assets wired.
- `validate_aos.sh .` 0 FAIL; `php -l` clean; per-route HTTP 200.
- Reusable QA: `scripts/qa/http-qa-axe.cjs`, `scripts/qa/http-qa-lighthouse.sh` (perf already on https), puppeteer probes in `scripts/qa/`.

## Standing governance / context
- **ADR034 offline-fallback:** named branches, NEVER main; file `roadmap.yaml` = SSoT (roadmap API hangs); merge → main + push **only on team_00 go**. main currently @ `c95b6d2`.
- **Cross-engine IR#1/#5:** Claude builds (team_10/team_80); validator engine ≠ builder → team_190 in Cursor.
- **DB probe** at startup; **validate_aos 0 FAIL** at every step; surgical per-file commits (never `git add -A`); `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`.
- **WP closure** per the contract: on L-GATE_VALIDATE PASS → team_191 ARCHIVE_MANIFEST → roadmap DONE (file SSoT, R9) → merge per go.
- Lessons (project memory `feedback_wp_staging_toolchain.md`): https-perf measurement, enqueue-handle silent-drop, orphaned-@media animation kill, equivalence-proof validation.

## FIRST ACTION (next session)
1. Canonical startup: read `_aos/roadmap.yaml`; DB probe `cat /Users/nimrod/Documents/agents-os/_aos/db_connectivity_status.json`; `validate_aos.sh .` → 0 FAIL.
2. Confirm B/E/F S2 sign-off with team_00 (proxy ok) — A is already signed.
3. Execute **Phase 0** (shared assets + context-driven partials) on a named branch; commit.
4. **Phase 1 — build cluster A first** (pattern-setter) per `LOD400_IMPL_spec`-A: prove the render-fn + context-driven partials; take A through S3→S4→deploy→S5.
5. **Phase 2 — once A's pattern is proven, fan out B/E/F in parallel** (3 sub-agents, isolation: worktree), each per its LOD400_IMPL_spec.
6. **Phase 3 — serialize** deploy → S4 → S5 across clusters; pre-flight each; route team_190 validates to Cursor (gate-order: team_50 PASS first).
7. Close+archive each cluster on PASS; merge to main per team_00 go. Report progress to team_00.
