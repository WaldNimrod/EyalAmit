...
| **WP-04 Crawl-policy + meta head pack** | CD (titles/meta must not inject body text that trips inventedSections); OV (breadcrumb UI no overflow) | **Schema/meta:** Google Rich Results Test + Schema.org validator on BreadcrumbList. **`<title>`/meta/canonical/OG/hreflang:** curl each route, assert one self-referencing canonical, 150–160-char meta, og:image 1200×630 resolves 200, reciprocal hreflang pairs, `/en` gets `lang=en dir=ltr`. **Per-post meta gap (verified this session):** individual blog posts currently emit **0** `<meta name="description">` (theme covers Pages + the `/blog/` archive only; Yoast emits none) — WP-04 must cover per-post meta, not just Pages. **Twitter/OG render:** card validators. **robots.txt:** the AI-bot 200 curl matrix (§5) — run the day robots.txt goes live, not bundled into monitoring. | One canonical/route; OG image 200; hreflang reciprocal; **AI UAs (OAI-SearchBot, ChatGPT-User, PerplexityBot, Claude-SearchBot, Claude-User, Google-Extended/GPTBot per D3) get HTTP 200, never 403** on 3 live URLs. **Reconcile the sitemap URL across robots `Sitemap:` directive, the legacy 301 target, and the GSC submission — all must name the SAME url, which is `/sitemap_index.xml`.** `/sitemap_index.xml` → 200 (Yoast); `/wp-sitemap.xml` → 301 → `/sitemap_index.xml`; live 301 maps `/מפת-אתר-site-map/ → /sitemap_index.xml`. The reconciled single sitemap URL across robots + GSC + legacy 301 = `/sitemap_index.xml`; legacy 301s to it are CORRECT. Verify all three references agree via a single-hop redirect-chain check. |
...
- **(c)** all **49 QR URLs → 200** — the `QR-URL-INVENTORY.csv` has 49 rows (parent `/qr/` + 48 `/qr/qrN/`), and asserting these **49 → 200** is CORRECT (do NOT rewrite to "48"). `/qr/פרק-א/` → 410 is NOT in the QR CSV; it is asserted via the 301/410-rules section (b), not here. **The real harness fix:** the QR check currently *follows redirects*, which would MASK the documented PROD 302 on parent `/qr/` (CSV row 1 notes `/qr/` 302s to `/shop/books/וכתבת/` on prod). Correct the assertion to require the QR URLs are **DIRECT 200 (no redirect-follow)** so that prod 302 cannot be masked. (team_100 is applying this fix in `scripts/final_pre_cutover_check.sh` separately.)
...
PRE-CUTOVER
  ...
  4. Build consolidated migration inventory (135-decision JSON + legacy mapping + GSC top
     ...
     QR counts: parent /qr/ + 48 /qr/qrN/ = 49 rows in QR-URL-INVENTORY.csv, all = 200 ;
     /qr/פרק-א/ = 410 (asserted via the 410-rules block, NOT the QR CSV).
...
DEPLOY + CONFIG TO PRODUCTION
  ...
  8. Publish the REAL production robots.txt (NOT hub/dist block-all); single agreed `Sitemap:` URL = `/sitemap_index.xml`.
...
| 6 | **QR surface** | parent `/qr/` + 48 `/qr/qrN/` = 49 CSV rows, all **DIRECT 200** (no redirect-follow, so the prod 302 on `/qr/` is not masked); `/qr/פרק-א/` = 410 | exact counts match, QR URLs direct-200 |
| 7 | **404 audit** | crawl the full migration inventory; every legacy URL is keep-200 / 301 / 410 — **zero unexpected 404** | 0 stray 404 |
| 8 | **Sitemap submit + resubmit** | submit the **single agreed sitemap URL `/sitemap_index.xml`** in GSC; confirm the robots `Sitemap:` directive + legacy 301 target + GSC entry all name `/sitemap_index.xml`; resubmit after deploy | sitemap accepted, no read errors, all name `/sitemap_index.xml` |
...
2. **Sitemap URL reconciliation** — `/sitemap_index.xml` → 200 (Yoast); `/wp-sitemap.xml` → 301 → `/sitemap_index.xml`. The single agreed URL across robots + 301 + GSC = `/sitemap_index.xml`. *Validation: robots + 301 + GSC name `/sitemap_index.xml`; single-hop chain check.*
3. **QR direct-200 assertion** — parent `/qr/` + 48 `/qr/qrN/` = 49 CSV rows, all 200 (49 → 200 is CORRECT, not "48"); `/qr/פרק-א/` = 410 via the 410-rules block. The real fix: assert the QR URLs are DIRECT 200 (no redirect-follow) so the documented prod 302 on `/qr/` cannot be masked. *Validation: `final_pre_cutover_check.sh` (c) asserts direct-200 on the 49 QR-URL-INVENTORY.csv rows.*
...
6. **D10 Moksha memorial content** — owner-dependent, content-only (slug is CLOSED at `/eyal-amit/mokesh-dahiman/`; only EYL-2 memorial content is open); blocks WP-07 + WP-12 memorial 301. Canonical/schema target = `/eyal-amit/mokesh-dahiman/`; `/about/moksha/` is a 301 SOURCE only. *Promote to day-one owner ask alongside D1/D6/D11.*
7. **Route-completeness for the machine layer** — `/shows/ /repair/ /bags/ /stand-floor/ /stands-storage/ /books/* /eyal-amit/mokesh-dahiman/ /press/` (`/about/moksha/` is a 301 source → /eyal-amit/mokesh-dahiman/, not a schema target) + 48 QR pages must get schema/meta/OG OR explicit noindex (WP-02/WP-04). *Validation: §3 enumerated-route audit.*
