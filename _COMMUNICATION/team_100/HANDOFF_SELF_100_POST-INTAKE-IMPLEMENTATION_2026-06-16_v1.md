...
- **Film (D-FILM — DECIDED 2026-06-19):** **(a) HERO** — the trailer `kf4NKSdYi9E` as a hero video: **autoplay, muted by default, unmute control, exactly like the home hero** (reuse `block-hero.php` + `ea-hero.js`, D-EYAL-VIDEO-13). **(b) Lower in page** — the **FULL FILM** for on-demand full viewing. ⚠ **Full-film link NOT public** (verified: only the trailer is on YouTube; the 1-hr film by Eyal & Guy Amit is on the FB movie page/screenings) → **awaiting full-film link from Eyal** (unlisted YT/Vimeo) or embed the FB movie page; ship hero trailer + marked placeholder until then. `VideoObject` JSON-LD for both (feeds SEO/GEO video schema). Full media spec: `MOKESH-MEDIA-CAPTURE-2026-06-16.md`.
