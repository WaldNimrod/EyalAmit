...
3. **Go-live is an unmanaged, irreversible operation.** Two stale 301 exports disagree with the live mu-plugin SSoT; the staging-noindex gate must be verified to lift; the canonical sitemap is **`/sitemap_index.xml`** (Yoast-served, HTTP 200; `/wp-sitemap.xml` 301s to it and the legacy `/מפת-אתר-site-map/` 301 already targets it — reconciled, DEC-SEO-B2 closed); and several real published routes (`/shows/ /repair/ /bags/ /stand-floor/ /stands-storage/ /books/* /eyal-amit/mokesh-dahiman/ /press/` + 48 `/qr/qr*/`) would ship as thin, schema-less, tagline-fallback pages.
...
Two hard reconciliations are carried as locked inputs (do not re-open): (1) **NAP/business name** = "המרכז לטיפול בנשימה באמצעות דיג'רידו" (cbDIDG), drop "סטודיו נשימה מעגלית" (full file:line enumeration in WP006 scope — incl. two SEED files needing re-seed), E.164 `+972-52-482-2842`, address עמל 8 ב' פרדס חנה, hours Sun–Thu 09–19 / Fri 09–14 / Sat closed / by-appointment, **SHOW address** (D1/D6 closed). (2) **Yoast is the live schema engine** (D4) — the theme/mu-plugin EXTENDS Yoast's single `@graph` via `wpseo_schema_graph`; never hand-roll a second `@graph`, never run two schema engines (see Appendix B).
...
| `S004-P001-WP002` | WP-02 | EXTEND Yoast's `@graph` via `wpseo_schema_graph` (Person, ProfessionalService, Service×route, FAQPage, Breadcrumb, ImageObject, VideoObject hooks) — single engine, no second graph; **dependency root**. See **Appendix B**. | P2 | A | P2 | M |
...
**Scope clarifications carried by the umbrella (supersede any "2 files" / "hand-rolled" framing in the exec plan):**

- **WP002 schema:** EXTEND Yoast's `@graph` (Person / ProfessionalService / Service-per-route / FAQPage / Breadcrumb / ImageObject / VideoObject) via the `wpseo_schema_graph` filter in `ea-w2-seo-schema.php` — a SINGLE `@graph`, never a second hand-rolled one (see **Appendix B**). WP002 also OWNS in-content link canonicalization (AC-18).
- **WP006 brand-string removal — FULL enumeration (verified live 2026-06-21, NOT "2 files").** "סטודיו נשימה מעגלית" appears in **6 live source files** (the `site/exports/blog-legacy.wxr` archive is a separate WXR import, out of theme scope):
  - `site/wp-content/themes/ea-eyalamit/template-parts/blocks/block-footer-social.php:64`
  - `site/wp-content/themes/ea-eyalamit/inc/wave2-w2-04-content.php:559, 729`
  - `site/wp-content/themes/ea-eyalamit/inc/wave2-w2-07.php:1017, 1020`
  - `site/wp-content/themes/ea-eyalamit/inc/wave2-w2-14e.php:420`
  - **SEED:** `site/wp-content/mu-plugins/ea-w2-02-core-pages-seed-once.php:246`
  - **SEED:** `site/wp-content/mu-plugins/ea-w2-07-qr-content-data.php:600, 649, 651, 653, 654`
  - **CRITICAL:** the two SEED files seed their output into the DB — a theme-file edit alone will NOT remove the string from rendered/seeded content. WP006 must **re-seed or run a content migration** for those routes; otherwise the AC-09 gate (`--absent 'סטודיו נשימה מעגלית' across all routes`) will FAIL on the seeded about/QR routes.
- **WP007 QR ownership:** WP007 OWNS the **48 `/qr/qrN/`** pages' raw-YouTube-iframe handling (facade / lazy-load / on-page transcript + CWV). These are the **largest video footprint** on the site. Source data: `site/wp-content/mu-plugins/ea-w2-07-qr-content-data.php`.
...
- **AC-07 — Crawl policy is correct and proven.** Production `site/` root `robots.txt` (not the block-all artifact) allows Googlebot/Bingbot + the AI citation UAs and declares the **single reconciled sitemap URL `/sitemap_index.xml`** (Yoast-served, HTTP 200; `/wp-sitemap.xml` 301s to it); the legacy sitemap 301 target (`/מפת-אתר-site-map/` → `/sitemap_index.xml`), the robots `Sitemap:` directive, and the GSC submission **all name `/sitemap_index.xml`** (DEC-SEO-B2 CLOSED). Post-cutover, `curl` of each named AI UA against ≥3 live URLs returns **HTTP 200** (not 403/blocked), run **the day robots goes live**, not deferred to the monitoring window. *(WP004/WP012/D3/DEC-SEO-B2)*
...
- **AC-09 — NAP locked byte-identical.** One canonical NAP string appears identically across footer, /contact, schema `#business`, and GBP; "סטודיו נשימה מעגלית" removed from **all** confirmed occurrences (NOT just the footer — see WP006 scope for the full file:line enumeration), with the explicit guard that the two SEED-file occurrences (`ea-w2-02-core-pages-seed-once.php`, `ea-w2-07-qr-content-data.php`) require a re-seed / content migration, not just a theme-file edit; the NAP-removal gate `--absent 'סטודיו נשימה מעגלית' across all routes` MUST pass on the seeded about/QR routes; schema `areaServed` equals the GBP service area. *(WP006/WP009/D1/D6)*
...
- **AC-11 — Full route inventory has machine layer or is explicitly noindexed.** Every published 200 route — including `/shows/ /repair/ /bags/ /stand-floor/ /stands-storage/ /books/vekatavta/ /books/kushi-blantis/ /eyal-amit/mokesh-dahiman/` (canonical memorial route; `/about/moksha/` and `/mokesh-dahiman/` are 301 SOURCES → it) `/press/` and the **48 `/qr/qr*/`** pages — either receives title/meta/canonical/OG (+ schema where applicable) or is explicitly marked noindex/out-of-scope; none ship with silent tagline-fallback meta. The 48 `/qr/qr*/` raw-iframe embeds are assigned to WP007 for facade/lazy/transcript remediation (see WP007 scope). *(SEO-B5, SEO-B3; WP002/WP004/WP005/WP007)*
- **AC-12 — End-to-end lead receipt proven (the single most consequential gate).** A real test lead submitted via **each** path (CF7 form, pre-filled `wa.me`, `tel:`) is confirmed **received by Eyal** (form lands in inbox, not spam) **and** the GA4 `generate_lead` event fires for each. CF7 confirmed active server-side; SMTP/deliverability + spam protection in place. **Named ops-task placeholder (owner/date TBD — team_00): activate CF7 + SMTP + server-side spam protection (honeypot/Turnstile)** — this is the only AC with no owned deadline and gates the entire go-live; it must be assigned and dated. *(WP001/WP013/SEO-B4/D7)*
- **AC-12b — Contact-path graceful degradation (regression guard).** `/contact/` ALWAYS exposes a tappable `wa.me` link + `tel:` even when `EA_WAVE2_CF7_FORM_ID = 0` / CF7 is inactive. This is **already true** in `site/wp-content/themes/ea-eyalamit/page-templates/tpl-contact.php`; this AC **formalizes the existing fallback and guards against regression** (do not treat it as a missing feature). *(WP001/SEO-B4)*
...
- **AC-17 — Track-A rollback criteria documented.** Each additive Track-A WP carries a documented **post-deploy REVERT TRIGGER** + a version-pin / git-revert path. Triggers include: malformed JSON-LD detected by Rich Results / Schema.org validator; a second/duplicate `generate_lead` event firing; any regression on the lead path. Highest-blast-radius WPs called out explicitly: **WP001** (sole conversion/lead path) and **WP002** (sitewide head/schema injection via `wpseo_schema_graph`). The `style.css` ver bump (cache-bust) is a deploy handle, not a rollback handle — the revert path must be a git revert / version pin. *(all Track-A WPs; WP001/WP002 highest priority)*
- **AC-18 — In-content links canonicalized (no 301 sources).** ZERO in-content links point to known-301 SOURCES (so FAQPage JSON-LD, which AC-05 requires to be byte-identical to the rendered DOM, cannot encode 301-bouncing links — that would violate AC-13 "every 301 = single hop, no chains"). Task assigned to **WP002** (schema-emitting surface) for in-content link canonicalization. **Confirmed live (verified 2026-06-21) in `site/wp-content/themes/ea-eyalamit/template-parts/blocks/block-faq-list.php`:** percent-encoded `/Blog/...` links at **lines 40, 105, 206** (301 SOURCE — `/Blog/` → `/blog/`) and a `/mokesh` link at **line 285** (301 SOURCE → `/eyal-amit/mokesh-dahiman/`). All must be repointed to canonical targets. *(WP002; gates AC-05/AC-13)*
- **AC-19 — Per-post meta description.** Individual blog posts currently emit **0** `<meta name="description">` (the theme covers Pages + the `/blog/` archive, not single posts, and Yoast emits none on these routes). WP004 head-pack scope MUST cover per-single-post meta description so no single post ships with empty/absent description. *(WP004)*
...
- **Pre-build blockers (resolve before/while building):** **SEO-B1** pillar URL buildability, ~~SEO-B2 sitemap URL contradiction~~ (**CLOSED** — sitemap reconciled at `/sitemap_index.xml`, DEC-SEO-B2), **SEO-B3** 48 QR raw-iframe ownership (assigned to WP007), **SEO-B4** CF7 server-gate + lead-receipt test, **SEO-B5** route enumeration gaps.
...
| D4 schema engine | **CLOSED** | **Yoast SEO v27.8 is the live schema engine**; theme/mu-plugin (`ea-w2-seo-schema.php`) EXTENDS it via the `wpseo_schema_graph` filter as a SINGLE `@graph`; never two graph engines; any competing plugin's schema module OFF. See Appendix B. |
...
| **D10 Mukesh memorial CONTENT** | **CONTENT-ONLY OPEN — PROMOTE TO DAY-ONE** | **Slug is CLOSED in live code: canonical = `/eyal-amit/mokesh-dahiman/`** (`/about/moksha/` and `/mokesh-dahiman/` are 301 SOURCES → it; schema nodes + 301 TARGET use `/eyal-amit/mokesh-dahiman/`). Only the memorial **CONTENT** remains OPEN/owner-gated (EYL-2: full-film link + facts/dates), owner-dependent (Eyal, unknown latency); content gates WP007 + WP012 |
...
| **DEC-SEO-B2 sitemap URL** | **CLOSED** | Canonical sitemap = **`/sitemap_index.xml`** (Yoast-served, HTTP 200; `/wp-sitemap.xml` 301s to it). The legacy 301 (`/מפת-אתר-site-map/` → `/sitemap_index.xml`) is CORRECT; robots `Sitemap:` + GSC submission all name `/sitemap_index.xml`. No 404, no `wp-sitemap.xml` mismatch (verified live 2026-06-21). See Appendix B |
...
| **Cutover regression** (301 chains, noindex stuck, thin routes) | HIGH | WP012 list-mode audit + AC-13/AC-14; sitemap reconciled at `/sitemap_index.xml` (DEC-SEO-B2 CLOSED); SEO-B3/B5 resolved pre-cutover |
| **Schema collision** (two engines) | MEDIUM | D4: Yoast is the single live schema engine; extend via `wpseo_schema_graph` (one `@graph`); never hand-roll a second graph; competing plugin schema OFF |
...
| **D10/EYL latency stalls the gated go-live** | MEDIUM | Promote D10 (content-only) to day-one owner ask; slug already CLOSED in live code at canonical `/eyal-amit/mokesh-dahiman/` (`/about/moksha/` + `/mokesh-dahiman/` are 301 SOURCES → it) |
...
- ~~DEC-SEO-B2 SITEMAP RECONCILIATION IS STILL UNDECIDED~~ **SUPERSEDED / CLOSED (verified live 2026-06-21, see Appendix B).** This earlier note assumed the site was theme-native WP-core (emitting `/wp-sitemap.xml`) and that `/sitemap_index.xml` 404s — both are **WRONG**. The live runtime is **Yoast SEO v27.8**, which serves `/sitemap_index.xml` at **HTTP 200**; `/wp-sitemap.xml` 301s to it; the legacy `/מפת-אתר-site-map/` → `/sitemap_index.xml` 301 (ea-w209-legacy-301-redirects.php:40) is **CORRECT**. DEC-SEO-B2 is CLOSED at `/sitemap_index.xml`; robots `Sitemap:` + GSC + the legacy 301 all name that one URL. No 404, no alias needed.
