...
**עדכון 14/7 — תבנית רביעית קבורה, אותו דפוס בדיוק:** team_00 ביקש לבדוק שוב את עמודי הספרים, ונמצא ש-**עמוד מוצר-ספר מלא, ברמת מסחר, כבר נבנה, עבר QA, אושר ע"י team_00 ונעל LOD500 — WP-W2-10-E** (`page-templates/tpl-book-detail.php` + `inc/wave2-w2-05.php:1026-1184`): כריכה אמיתית, מחיר-על-הכפתור, אקורדיון "קטע לקריאה", CTA מפוצל דפוס/e-book, גלריה, FAQ. **גם הוא קבור** — Chapters מנצחת בניתוב, וה-Chapters version (`vekatavta/kushi-blantis/tsva-bekahol-defaults.php`) הוא מימוש שני, עצמאי, לאותו תפקיד בדיוק. נוסף כ-T3b למטה. (לעומת זאת: Book schema — נבדק פעם שלישית לעומק — **אכן לא קיים בשום מקום**, לא חי ולא מת; הוא כן תוכנן במסמך אסטרטגיית ה-SEO המקורי של אייל מאפריל אך מעולם לא מומש. T4 למטה נשאר נכון כפי שהוא.)
...
| T3b | **חדש** — Reconcile עמודי ספר | WP-W2-10-E (`tpl-book-detail.php`+`wave2-w2-05.php:1026-1184`, LOD500_LOCKED, QA+team_00 signoff מ-2/6) מול הגרסה המקבילה ב-Chapters (`*-defaults.php`) — להשוות תכונה-מול-תכונה (כריכה+מחיר-על-הכפתור, CTA מפוצל דפוס/e-book, גלריה), להעביר כל מה שחסר ב-Chapters מהגרסה המאושרת, ואז Chapters מנצחת. | אין |
| T4 | Schema חדש | FAQPage (על גבי T2), Book (על כל 3 עמודי הספרים) — לא קיימים כלל, לא רק "לא מנותבים" (מתוכנן במקור במסמך אסטרטגיית SEO של אייל מ-23/4, מעולם לא מומש). | T2, T3b |
| T5 | איחוד `/shop` + `/qr` | ה-routes האחרונים שעדיין על Wave2 בפועל. | T1-T3 |
| T6 | מחיקת Wave2 | כל `tpl-service.php`, `template-treatment.php`, `template-method.php`, `tpl-faq.php`, `template-faq-catalog.php`, `tpl-shop-item.php`, `tpl-shop-archive.php`, `tpl-book-detail.php`, `tpl-contact.php`, `tpl-home.php`, `template-home-dashboard.php`, ו-`inc/wave2-w2-*.php` שהופכים מיותרים — רק לאחר וידוא אחד-אחד שאין תלות חיה. | T1-T5, T3b |
