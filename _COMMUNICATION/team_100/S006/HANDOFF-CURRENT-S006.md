...
**עודכן:** 2026-08-18 (סשן 6) · **שלב פעיל:** 1 (ליבה, דסקטופ בלבד)
...
זה מקור העדכון האחרון לכל עמוד וענף — החלטות, הגדרות, הערות אייל — **כולל עמודים שכבר הוגשו**. לא כותבים מ-`docs/project/eyal-ceo-submissions-and-responses/from-eyal/`. לא פותחים מחדש סעיפים ממתינים לאייל (H-01/H-06/H-07, T-01/T-02, MTH-01, LSN-01/LSN-02/LSN-09, M-01a/M-03/M-04/M-05) בלי סקואופ חדש; בבדיקה הבאה של אותם עמודים הקליטה היא מהתיקייה הזו.
...
R1-04 `/lessons/` **הוגש** (חוזה + E2E Composer PASS). הבא בתור להצעת סקואופ (לא לביצוע בלי אישור כתוב):

| # | שורה | נתיב | חומר מאייל |
|---|---|---|---|
| הבא | R1-05 | `/sound-healing/` | `content 13.8.26/סאונדהילינג/` |
| אחריו | R1-06..R1-09 | `/learning/` + 3 תת־עמודים | — |
| אחריו | R1-10 | `/shop/` | `content 13.8.26/כלים למכירה/` |

המשך בטרקר, טאב `סבב-1-ליבה`. **אין לפתוח `/sound-healing/` בלי `סקואופ אושר: R1-05 בלבד`.**
...
## ב.3 ממתין לאייל — 13 סעיפים, כולם כתובים כשאלה אחת עם בחירה מרשימה
