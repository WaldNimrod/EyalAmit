...
**עודכן:** 2026-08-17 (סשן 4) · **שלב פעיל:** 1 (ליבה, דסקטופ בלבד)
...
R1-02 **הוגש**. הבא בתור להצעת סקואופ (לא לביצוע בלי אישור כתוב):

| # | שורה | נתיב | חומר מאייל |
|---|---|---|---|
| הבא | R1-03 | `/method/` | `content 13.8.26/השיטה/` |
| אחריו | R1-04 | `/lessons/` | `content 13.8.26/שיעורי נגינה/` |
...
המשך בטרקר, טאב `סבב-1-ליבה`. **אין לפתוח `/method/` בלי `סקואופ אושר: R1-03 בלבד`.**
...
## ב.3 ממתין לאייל — 9 סעיפים, כולם כתובים כשאלה אחת עם בחירה מרשימה
