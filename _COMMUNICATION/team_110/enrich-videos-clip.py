#!/usr/bin/env python3
"""Tag DOK-WEB video thumbnails using CLIP (via HuggingFace transformers).

Output: hub/dist/files/team40/video-clip-tags.json
  { "VID-BQSS0835": ["דיגרידו", "נגינה", ...], ... }

Run: python3 _COMMUNICATION/team_110/enrich-videos-clip.py
"""
import csv, json, os
...
THUMB_DIR  = f"{DOK}/video_thumbs"
VID_CSV    = f"{DOK}/_INDEX_VIDEOS.csv"
OUT_JSON   = f"{PROJ}/_COMMUNICATION/team_110/build/enrichment/video-clip-tags.json"
SCORE_THRESH = 0.18
...
def main():
    ...
    for row in rows:
        ...
        vid_id = f"VID-{stem}"
        thumb  = os.path.join(THUMB_DIR, f"{stem}.jpg")
        if vid_id not in existing and os.path.exists(thumb):
            todo.append((vid_id, thumb))

    total = len(todo)
    missing = len(rows) - total - len(existing)
    print(f"To process: {total} / {len(rows)}  (missing thumbs: {missing})")

    for i, (vid_id, thumb) in enumerate(todo, 1):
        tags = tag_image(thumb, model, processor, device)
        existing[vid_id] = tags
    ...
    with_tags = sum(1 for v in existing.values() if v)
    print(f"\nDone. {len(existing)} videos tagged, {with_tags} with visual tags → {OUT_JSON}")
