...
function ea_chapters_phero_overlay() {
	...
	/* S006 R1-02…R1-05 + wave1 R1-10/16/21/22 · seed from PHP defaults so ACF
	 * slots from the previous section order cannot overwrite. */
	if ( in_array( ea_chapters_type(), array( 'treatment', 'method', 'lessons', 'sound-healing', 'shop', 'muzza', 'about', 'mokesh' ), true ) ) {
...
function ea_chapters_page_sections() {
	...
	foreach ( $secs as $n => $sec ) {
		...
		/* S006 R1-02…R1-05 + wave1 R1-10/16/21/22 · seeded defaults only (see phero overlay). */
		if ( in_array( $type, array( 'treatment', 'method', 'lessons', 'sound-healing', 'shop', 'muzza', 'about', 'mokesh' ), true ) ) {
