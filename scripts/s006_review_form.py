...
def _status_for_eyal_html(model) -> str:
    ...
    return (
        ...
        "הבדיקה, בעמוד אחד שינינו משהו קצת אחרת ממה שביקשת, ובשני נושאים גילינו שמה שביקשת כבר "
        ...
        "«לא טוב בלי נושא בכלל.» השדה נשאר, אבל **כרשימה נפתחת** במקום כתיבה חופשית, "
        "וכותרת המייל שמגיע אליך נבנית מהנושא שנבחר. יש למטה שאלה עם רשימת הנושאים "
        "המוצעת — נשמח שתאשר אותה או תתקן.</li>\n"
        "</ul>\n"
        "<h3>מה שעוד פתוח אצלנו</h3>\n"
        "<ul>\n"
        "<li><strong>שני כפתורי הוואטסאפ</strong> בעמוד צור קשר — צדקת, יש שניים, "
        "ועדיין לא טיפלנו. אין צורך שתעיר על זה שוב.</li>\n"
        "</ul>\n"
