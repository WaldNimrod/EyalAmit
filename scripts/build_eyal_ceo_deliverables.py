...
def build_executive_summary_docx(out_dir: Optional[Path] = None):
    ...
    add_rtl_paragraph(doc, f"גרסה: 2.0 (מסונכרן אפיון סופי + תקציר MD v2.0)  |  תאריך: {DELIVERY_DATE}")
...
1. פלטפורמה (נעול): WordPress בלבד — כל התוכן בתוך WordPress; אין חלק מהאתר מחוץ ל-WordPress. "רזה" = התקנה נקייה, תבנית מודרנית רזה ותקינה, מינימום תוספים (מוקפים ונחוצים), ניקוי פלונטרים של הפניות וקישורים שבורים, ועדכון עץ האתר לפעילותך. (ארכיון כאן = מסלול פיתוח קודם — לא לבלבל עם מבנה תוכן של ספרים.)
...
4. ספרים, הוצאה והופעות: ספרים והוצאה — מדור פעיל במבנה האתר (תפריט ראשי ותוכן), לפי מפת האתר (מוזג תחת "ספרים"; לא כ"ארכיון" ראשי). הופעות ומורשת מופע — ניווט משני (חוויית ארכיון לעבר), לא חזית המותג המרכזית — כפי שסוכם במשוב.

5. חנות / קטלוג: ללא עגלה ותשלום באתר; עמוד קטלוג ראשי עם שמירת נתיב קיים (לא 301 תחתון במקום עמוד); חשבונית ירוקה לסליקה חיצונית.
